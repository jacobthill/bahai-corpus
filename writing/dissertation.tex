...
All three techniques have been used as a means of author detection while the first two have also been used, cautiously, to measure the writing ability of authors, the assumption being that their is a correlation between these statistics and a writer's knowledge of the language. In some cases word length has been combined with other language specific data resulting in more effective models for specific circumstances. For example, Renjui and Chu combine word length with word-final tones motifs and segment-final motifs to improve author prediction in Chinese texts \cite{Renkui and Chu}.
Given that author detection is not the present objective, we will see what, if anything, these methods can teel us about the Bah\'{a}'u'llah's writing style. But first...
\par
Word length in Arabic, however, has one problem that should be noted. The partical `waw`, a single letter meaning and, is frequently found in all Arabic writing. Using waw frequently would not have the same effect in Arabic as the frequent use of `and' in English. The former is perfectly acceptable, whereas, the latter would be considered poor style. Given the lack of punctuation in classical Arabic the `waw` often functioned as a combination of the English `and', as well as `comma', and `period`. The problem for determining word length is that it is inconsistently adjoined with the succeeding word without a space in between them. Thus instead of having many words of length one, a significant portion of these `waws' are merged with other words. This, like the oxford comma in English, could be an effect of either author style or a side effect introduced when the physical text was made digital. This is consistently the case with the writings of the B\'{a}b sampled here-there are no words of lenght one-and inconsistently the case with the writings of al-Shaykh Murtaḍ\'{a} al-\'{A}ns\'{a}r\'{i}}, indicating probably the compilation of his writings from different sources. The impact of this is unclear and beyond the scope of this study, given that author detection is not the objective. This point does serve, however, as another example of the need for careful consideration of the history and circumstances surrounding the development of a language and the production of literary works.
